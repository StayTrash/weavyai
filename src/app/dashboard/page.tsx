...
import type { Workflow, Folder } from '@/components/dashboard/types';

/**
 * DashboardPage Component
 * Uses Clerk for authentication and server actions for data fetching.
 */
export default function DashboardPage() {
  ...
  const workflowsQuery = useQuery({
    ...
    queryFn: () => listWorkflows({ folderId: currentFolderId }),
    enabled: !!isSignedIn,
  });
  const foldersQuery = useQuery({
    queryKey: ['folders', currentFolderId],
    queryFn: () => listFolders({ parentId: currentFolderId }),
    enabled: !!isSignedIn,
  });
  const allFoldersQuery = useQuery({
    queryKey: ['folders', null],
    queryFn: () => listFolders({ parentId: null }),
    enabled: moveDialogOpen && !!isSignedIn,
  });

  // Mutations (server actions)
  const createWorkflowMutation = useMutation({
    mutationFn: (input: { name?: string; folderId?: string | null }) => createWorkflow(input),
    onSuccess: (data) => {
      router.push(`/dashboard/workflow/${data.workflow.id}`);
    },
  });
  const updateWorkflowMutation = useMutation({
    mutationFn: (input: { id: string; name?: string; folderId?: string | null }) => updateWorkflow(input),
    onSuccess: () => {
  ...
  const deleteWorkflowMutation = useMutation({
    mutationFn: (input: { id: string }) => deleteWorkflow(input),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['workflows'] });
    },
  });
  ...
  const updateFolderMutation = useMutation({
    mutationFn: (input: { id: string; name?: string; parentId?: string | null }) => updateFolder(input),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['folders'] });
    },
  });
  const deleteFolderMutation = useMutation({
    mutationFn: (input: { id: string }) => deleteFolder(input),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['folders'] });
    },
  });

  // Derived data (server actions return { workflows } / { folders })
  ...
  }, [workflows, currentFolderId, createWorkflowMutation, queryClient]);
